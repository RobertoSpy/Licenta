  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3 mb-8">
        ...
        <h1 className="text-3xl font-bold text-slate-800 dark:text-white">Cereri de Ofertă (Lead-uri)</h1>
      </div>

      {loading ? (
        <div className="space-y-4 animate-pulse">
          {[1,2,3].map(i => <div key={i} className="h-24 bg-slate-200 dark:bg-slate-800 rounded-xl"></div>)}
        </div>
      ) : quotes.length === 0 ? (
        <div className="bg-white dark:bg-slate-800 p-12 rounded-2xl border border-slate-200 dark:border-slate-700 text-center">
          <p className="text-slate-500 dark:text-slate-400">Nu ai primit nicio cerere de ofertă încă.</p>
        </div>
      ) : (
            ...
            {quotes.map(q => (
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <h4 className="font-bold text-slate-900 dark:text-white truncate">Proiect #{q.projectId}</h4>
            ...
            <AnimatePresence mode="wait">
              {selectedQuote ? (
                <motion.div
                  ...
                  exit={{ opacity: 0, x: -20 }}
                  className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm"
                >
                  <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-6 flex items-center justify-between">
                    <span>Detalii Proiect #{selectedQuote.projectId}</span>
                    <span className="text-sm font-normal px-3 py-1 bg-slate-100 dark:bg-slate-700 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 gap-6 mb-8">
                    ...
                    <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-xl flex flex-col items-center justify-center text-center">
                  ...
                  {selectedQuote.status === 'PENDING' && isVerified !== false ? (
                    <form onSubmit={handleSubmitQuote} className="space-y-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          ...
                          <input 
                            ...
                            className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-blue-500 font-mono text-lg"
                      ...
                      <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Mesaj pentru client</label>
                        <textarea 
                          rows={3} 
                          className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-blue-500"
                      ...
                      <div className="flex items-center gap-3 p-4 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700">
                        ...
                        />
                        <label htmlFor="acceptsBOM" className="text-sm font-medium text-slate-700 dark:text-slate-300 cursor-pointer">
                      ...
                      <div className="pt-4 flex justify-end">
                        <button
                          ...
                          className="px-8 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-colors disabled:opacity-50 flex items-center gap-2"
                        >
                    ...
                    <div className="bg-slate-50 dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-700">
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <p><strong>Mesaj:</strong> {selectedQuote.message || '-'}</p>
                      </div>
                    </div>
                  )}
                ...
                <motion.div
                  ...
                  className="h-full min-h-[400px] flex flex-col items-center justify-center text-slate-400 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl"
                >
