...
import { squarifyPartition } from './treemap';
...
export function generateConfiguratorLayout(
  ...
  const GAP_TOLERANCE = ARCHITECTURAL_STANDARDS.WALL.INTERIOR_THICKNESS + 0.05; // ~0.175m
    ...
    for (const neighborP of partitionedRooms) {
      ...
      if (boundary) {
        ...
        if (!bestNeighbor || isHol) {
          ...
          if (isHol) break; // Am găsit vecinul ideal
        }
      }
    }
