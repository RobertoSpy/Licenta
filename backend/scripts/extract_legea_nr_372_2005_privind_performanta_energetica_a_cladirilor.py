...
def main() -> int:
    pdf_path = Path(
        sys.argv[1]
        if len(sys.argv) > 1
        else Path(__file__).resolve().parents[1] / "docsAI" / "legea-nr-372-2005-privind-performanta-energetica-a-cladirilor.pdf"
    )
    output_path = Path(
        sys.argv[2]
        if len(sys.argv) > 2
        else Path(__file__).resolve().parents[1] / "docsAI" / "legea-nr-372-2005-privind-performanta-energetica-a-cladirilor.md"
    )
