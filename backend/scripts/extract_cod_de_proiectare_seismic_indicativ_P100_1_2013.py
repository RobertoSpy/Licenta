...
def main() -> int:
    pdf_path = Path(
        sys.argv[1]
        if len(sys.argv) > 1
        else Path(__file__).resolve().parents[1] / "docsAI" / "cod-de-proiectare-seismic-indicativ-P100-1-2013.pdf"
    )
    output_path = Path(
        sys.argv[2]
        if len(sys.argv) > 2
        else Path(__file__).resolve().parents[1] / "docsAI" / "cod-de-proiectare-seismic-indicativ-P100-1-2013.md"
    )
