...
def main() -> int:
    pdf_path = Path(
        sys.argv[1]
        if len(sys.argv) > 1
        else Path(__file__).resolve().parents[1] / "docsAI" / "Monitorul Oficial Partea I nr. 53Bis - Ordin + anexa__NE 012-1-2022.pdf"
    )
    output_path = Path(
        sys.argv[2]
        if len(sys.argv) > 2
        else Path(__file__).resolve().parents[1] / "docsAI" / "Monitorul Oficial Partea I nr. 53Bis - Ordin + anexa__NE 012-1-2022.md"
    )
