...
def main() -> int:
    pdf_path = Path(
        sys.argv[1]
        if len(sys.argv) > 1
        else Path(__file__).resolve().parents[1] / "docsAI" / "DECRET-LEGE 114 30_03_1990 - Portal Legislativ.pdf"
    )
    output_path = Path(
        sys.argv[2]
        if len(sys.argv) > 2
        else Path(__file__).resolve().parents[1] / "docsAI" / "DECRET-LEGE 114 30_03_1990 - Portal Legislativ.md"
    )
