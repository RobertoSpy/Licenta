...
exports.calcHouseFootprint = calcHouseFootprint;
exports.buildRoomLayout = buildRoomLayout;
// ─────────────────────────────────────────────────────────────────
// BFS pe layout_matrix → identifică regiunile rectangulare conectate
// ─────────────────────────────────────────────────────────────────
function extractRegions(matrix) {
    const rows = matrix.length;
    const cols = matrix[0].length;
    const visited = Array.from({ length: rows }, () => new Array(cols).fill(false));
    const regions = [];
    const bfs = (startRow, startCol, label) => {
...
// ─────────────────────────────────────────────────────────────────
function calcHouseFootprint(totalSqm) {
...
// ─────────────────────────────────────────────────────────────────
function buildRoomLayout(matrix, ratios, house) {
    var _a;
    const rows = matrix.length;
    const cols = matrix[0].length;
    ...
    const rawRooms = regions.map((region) => {
    ...
    for (const r of rawRooms) {
        labelCounts.set(r.label, ((_a = labelCounts.get(r.label)) !== null && _a !== void 0 ? _a : 0) + 1);
    }
...
// ─────────────────────────────────────────────────────────────────
function adjustByColumn(rooms, ratios, totalArea, house) {
    var _a;
    ...
    for (const [colX, colRooms] of colGroups) {
        ...
        const sumRatios = sorted.reduce((sum, r) => { var _a; return sum + ((_a = ratios[r.label]) !== null && _a !== void 0 ? _a : 0.1); }, 0);
        let currentY = house.yM;
        for (const room of sorted) {
            const ratio = (_a = ratios[room.label]) !== null && _a !== void 0 ? _a : 0.1;
