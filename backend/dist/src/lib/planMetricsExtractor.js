...
exports.extractMetricsFromSnapshot = extractMetricsFromSnapshot;
// ─────────────────────────────────────────────────────────────────
...
function isExteriorDoor(door, bbox) {
    var _a, _b;
    // Dacă metadata spune explicit → prioritate
    if (((_a = door.metadata) === null || _a === void 0 ? void 0 : _a.isExterior) === true)
        return true;
    if (((_b = door.metadata) === null || _b === void 0 ? void 0 : _b.isExterior) === false)
        return false;
    ...
    return (cx <= bbox.minX + EXTERIOR_THRESHOLD_PX ||
...
function openingAreaSqm(el) {
    ...
    if (wM < 0.1 || hM < 0.1)
        return 0; // dimensiune degenerat-0, ignorăm
 ...
 */
function extractMetricsFromSnapshot(planJSON, floorsCount, foundationDepthM, foundationWidthM) {
    ...
    const exteriorOpeningsSqm = parseFloat((windows.reduce((s, w) => s + openingAreaSqm(w), 0) +
        exteriorDoors.reduce((s, d) => s + openingAreaSqm(d), 0)).toFixed(2));
...
// ─────────────────────────────────────────────────────────────────
function buildFallback(floorsCount, foundationDepthM, foundationWidthM, note) {
    return {
        fromSnapshot: false,
        diagnosticNote: note,
        metrics: {
