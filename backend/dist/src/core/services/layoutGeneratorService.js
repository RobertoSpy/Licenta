...
exports.LayoutGeneratorService = void 0;
...
class LayoutGeneratorService {
    static generateLayout(request) {
        const styleKey = this.normalizeStyle(request.style);
        // 1. Găsește template-ul
        const template = house_styles_json_1.default.templates[styleKey];
        if (!template) {
        ...
        const config = template.configurations[configKey] || template.configurations['1_dormitor'];
        // 3. Calculează footprint-ul dreptunghiular
        const footprint = (0, treemapPartitioner_1.calcHouseFootprint)(request.totalFloorAreaSqm);
        ...
        const rawRooms = (0, treemapPartitioner_1.buildRoomLayout)(config.layout_matrix, config.ratios, baseRect);
        // 5. Aplică constrângerile legale (Solver)
        const legalRooms = (0, layoutConstraintSolver_1.applyLegalConstraints)(rawRooms);
    ...
    static normalizeStyle(style) {
        ...
        if (s.includes('modern'))
            return 'Modern';
        if (s.includes('industrial') || s.includes('loft'))
            return 'Industrial';
        if (s.includes('clasic') || s.includes('traditional'))
            return 'Clasic';
        if (s.includes('mediteranean'))
            return 'Mediteranean';
        if (s.includes('rustic'))
            return 'Rustic';
        return 'Modern'; // fallback
    }
    static convertToCanvasElements(rooms, footprint) {
        const elements = [];
...
exports.LayoutGeneratorService = LayoutGeneratorService;
