...
exports.VALID_FLOORS = exports.VALID_ZONES = exports.VALID_ROOM_TYPES = void 0;
exports.buildRoomProgramPrompt = buildRoomProgramPrompt;
exports.validateRoomSuggestion = validateRoomSuggestion;
const conformity_rules_json_1 = __importDefault(require("../../../data/conformity-rules.json"));
exports.VALID_ROOM_TYPES = ['hol', 'living', 'bucatarie', 'dormitor', 'baie', 'wc', 'camara', 'birou', 'sala_mese', 'terasa', 'debara'];
exports.VALID_ZONES = ['distributie', 'zi', 'noapte', 'tehnic'];
exports.VALID_FLOORS = ['parter', 'etaj1'];
function buildRoomProgramPrompt(params) {
...
${conformity_rules_json_1.default.room_min_sqm.map((r) => `- ${r.targets.join('/')}: minim ${r.min_sqm} mp`).join('\n')}
...
Tipuri valide 'type': ${exports.VALID_ROOM_TYPES.join(', ')}
Zone valide: ${exports.VALID_ZONES.join(', ')}
Floor valide: ${exports.VALID_FLOORS.join(', ')}
weightRatio: 0.5 (mic) → 4.0 (mare)`;
}
function normalizeLabel(label) {
    return (label !== null && label !== void 0 ? label : '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/\s+/g, '').replace(/[^a-z0-9]/g, '');
}
function validateRoomSuggestion(parsed, targetArea) {
    ...
    for (const room of parsed.rooms) {
        const rule = conformity_rules_json_1.default.room_min_sqm.find((r) => r.targets.includes(normalizeLabel(room.type)));
