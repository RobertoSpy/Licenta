...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const mat of BASELINE_MATERIALS) {
            const existing = yield prisma.material.findUnique({
        ...
        for (const group of alternativeGroups) {
            const materialsInDB = yield prisma.material.findMany({
        ...
        try {
            const syncRes = yield scraperService_1.scraperService.syncAllMaterials(true);
        ...
        for (const [key, val] of Object.entries(formulas)) {
            if (key === '_meta')
                continue;
            if (val.defaultMaterialCode)
                requiredCodes.add(val.defaultMaterialCode);
        }
        let missingCount = 0;
        for (const code of requiredCodes) {
            const mat = yield prisma.material.findUnique({ where: { internalCode: code } });
            if (!mat) {
